...
function RankBadge({ rank, size = 36 }) {
  if (rank === 1) {
    return (
      <div className="relative flex items-center justify-center mx-auto shrink-0" style={{ width: size, height: size }}>
        <Gear size={size} rotation={0} color="#FFB000" className="animate-[spin_20s_linear_infinite]" />
        <span className="absolute font-black text-xs text-[#0B1B3A] z-10">1</span>
      </div>
    )
  }
  if (rank === 2) {
    return (
      <div className="relative flex items-center justify-center mx-auto shrink-0" style={{ width: size, height: size }}>
        ...
        <span className="absolute font-black text-xs text-[#0B1B3A] z-10">2</span>
      </div>
    )
  }
  if (rank === 3) {
    return (
      <div className="relative flex items-center justify-center mx-auto shrink-0" style={{ width: size, height: size }}>
        ...
        <span className="absolute font-black text-xs text-[#0B1B3A] z-10">3</span>
      </div>
    )
  }
  return (
...
function MechanicalGearsMini({ scrollProgress }) {
  ...
  return (
    <div className="flex items-center gap-0.5 px-2 py-1 bg-slate-100 border border-slate-300 rounded-full shadow-inner select-none pointer-events-none shrink-0" title="Transmission telemetry linked to scroll">
      {/* Gear 1: Clockwise */}
...
export default function Leaderboard() {
  ...
  const fetchRankings = async () => {
    try {
      ...
      try {
        ...
        console.warn('[Leaderboard] Warning: Could not fetch leaderboard config:', e.message)
      }
      ...
      try {
        const { data: directProds, error: directProdsErr } = await supabase
          ...
          .eq('status', 'active')
        if (!directProdsErr && Array.isArray(directProds) && directProds.length > 0) {
        ...
        console.warn('[Leaderboard] Warning: Could not fetch active products metadata:', e.message)
      }
      ...
      const resolvedTeams = teamList.map(t => {
        if (!t) return null;
        const teamProducts = (productsData || []).filter(p => p && p.team_id === t.id);
        const resolvedDomain = teamProducts.length > 0 && teamProducts[0]?.innovation_domain
          ? teamProducts[0].innovation_domain
          : (t.innovation_domain || 'Open Innovation');

        let leadingTitle = null;
        let calculatedHighestTrl = (t.highest_trl !== undefined && t.highest_trl !== null)
        ...
        if (teamProducts.length > 0) {
          ...
          const prodsWithTrl = teamProducts.filter(p => (p.trl_level !== undefined && p.trl_level !== null) || (p.current_trl !== undefined && p.current_trl !== null));
          if (prodsWithTrl.length > 0) {
            const maxPTrl = Math.max(...prodsWithTrl.map(p => Number(p.trl_level ?? p.current_trl)));
            calculatedHighestTrl = calculatedHighestTrl !== null ? Math.max(calculatedHighestTrl, maxPTrl) : maxPTrl;
          }
        }
        ...
        return {
          ...
          department: t.department || t.department_name || 'Unknown Department',
          innovationDomain: resolvedDomain || 'Open Innovation',
          ideas: t.ideas ?? t.ideas_count ?? 0,
          highestTrl: calculatedHighestTrl,
          earliestTime: t.earliest_time || t.earliest_submission_time || t.created_at || null,
          leadingProductTitle: leadingTitle || t.leading_product_title || null
        };
      }).filter(Boolean)

      resolvedTeams.sort((a, b) => {
        const trlA = a.highestTrl !== null && a.highestTrl !== undefined ? Number(a.highestTrl) : -1;
        const trlB = b.highestTrl !== null && b.highestTrl !== undefined ? Number(b.highestTrl) : -1;
        if (trlB !== trlA) {
          return trlB - trlA;
        }
      ...
      setError(err.message || 'Unable to compute mechanical leaderboard rankings.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const renderOverallView = () => {
    const departments = Array.from(new Set(overallStats.allTeams.map(t => t.department).filter(Boolean))).sort((a, b) => String(a ?? '').localeCompare(String(b ?? '')))
    ...
    const activeTrls = Object.keys(trlDistribution).filter(k => trlDistribution[k] > 0).map(Number);
    ...
    const trlConfigs = [
      { level: 1, label: "TRL 1", desc: "(Basic)", colorClass: "bg-blue-500/20 text-blue-700 border border-blue-400/50 hover:bg-blue-500/30" },
      { level: 2, label: "TRL 2", desc: "(Formulated)", colorClass: "bg-blue-500/20 text-blue-700 border border-blue-400/50 hover:bg-blue-500/30" },
      { level: 3, label: "TRL 3", desc: "(Proof of Concept)", colorClass: "bg-blue-500/25 text-blue-800 border border-blue-450/50 hover:bg-blue-500/35" },
      { level: 4, label: "TRL 4", desc: "(Validated)", colorClass: "bg-emerald-500/20 text-emerald-700 border border-emerald-400/50 hover:bg-emerald-500/30" },
      { level: 5, label: "TRL 5", desc: "(Prototype)", colorClass: "bg-emerald-500/20 text-emerald-700 border border-emerald-400/50 hover:bg-emerald-500/30" },
      { level: 6, label: "TRL 6", desc: "(System)", colorClass: "bg-emerald-500/25 text-emerald-800 border border-emerald-450/50 hover:bg-emerald-500/35" },
      { level: 7, label: "TRL 7", desc: "(Demonstration)", colorClass: "bg-amber-500/20 text-amber-700 border border-amber-400/50 hover:bg-amber-500/30" },
      { level: 8, label: "TRL 8", desc: "(Pre-Production)", colorClass: "bg-amber-500/25 text-amber-800 border border-amber-450/50 hover:bg-amber-500/35" },
      { level: 9, label: "TRL 9", desc: "(Market Ready)", colorClass: "bg-orange-500/35 text-orange-800 border border-orange-400/50 hover:bg-orange-500/45" }
    ];
    ...
    const hasTrlData = leaderboardType === 'TRL_BASED' || (overallStats.allTeams.length > 0 && overallStats.allTeams.some(t => t.highestTrl !== undefined && t.highestTrl !== null));
            ...
            <div>
              ...
              <div className="space-y-4 mt-2">
                <div className="flex items-center justify-between text-sm py-2.5 px-3.5 border border-slate-200 bg-slate-50 rounded-xl">
                  <span className="text-slate-700 font-bold uppercase tracking-wider text-[10px]">Teams Registered</span>
                  <span className="font-black text-[#0B1B3A] text-base">{kpis.totalTeams}</span>
                </div>
                <div className="flex items-center justify-between text-sm py-2.5 px-3.5 border border-slate-200 bg-slate-50 rounded-xl">
                  <span className="text-slate-700 font-bold uppercase tracking-wider text-[10px]">Ideas Submitted</span>
                  <span className="font-black text-[#0B1B3A] text-base">{kpis.totalIdeas}</span>
                </div>
              </div>
            </div>
          ...
          <div className="relative lg:col-span-2 bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm flex flex-col justify-between overflow-hidden min-w-0">
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 mb-6 gap-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <label htmlFor="dept-filter-chart" className="text-[11px] font-black text-slate-700 uppercase tracking-wider">Dept Selector:</label>
                  <select
              ...
              <div className="w-full min-w-0">
                <div className="flex justify-between items-end h-44 sm:h-48 w-full px-1 pb-2 relative gap-1 sm:gap-2">
                  ...
                  {trlConfigs.map(cfg => {
                    ...
                    return (
                      <div key={cfg.level} className="flex flex-col items-center gap-1.5 sm:gap-2 flex-1 min-w-0 relative z-10">
                        <span className="text-[9px] sm:text-[10px] font-black text-[#0B1B3A] font-mono">{count}</span>
                        ...
                        <div className="flex flex-col items-center text-center w-full px-0.5">
                          <span className="text-[8px] sm:text-[10px] font-black text-[#0B1B3A] tracking-wider truncate w-full">{cfg.label}</span>
                          <span className="hidden sm:block text-[7px] sm:text-[8px] font-bold text-slate-500 uppercase tracking-wider truncate w-full">{cfg.desc}</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 grid grid-cols-2 min-[440px]:grid-cols-3 gap-3 sm:gap-4">
              <div className="flex flex-col bg-slate-50 p-3 rounded-xl border border-slate-200">
                <span className="text-[10px] font-black text-slate-600 uppercase tracking-wider">Total Teams</span>
                <span className="text-lg sm:text-xl font-black text-[#0B1B3A] mt-1">{chartTotalTeams}</span>
              </div>
              <div className="flex flex-col bg-slate-50 p-3 rounded-xl border border-slate-200">
                <span className="text-[10px] font-black text-slate-600 uppercase tracking-wider">Highest TRL</span>
                <span className="text-lg sm:text-xl font-black text-[#0B1B3A] mt-1">{highestTrl > 0 ? `TRL ${highestTrl}` : 'N/A'}</span>
              </div>
              <div className="flex flex-col bg-amber-500/10 p-3 rounded-xl border border-amber-300/70 col-span-2 min-[440px]:col-span-1">
                <span className="text-[10px] font-black text-amber-700 uppercase tracking-wider">Top TRL Range</span>
                <span className="text-lg sm:text-xl font-black text-[#0B1B3A] mt-1">{topRange}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="relative bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm overflow-hidden lg:col-span-1 min-w-0">
            ...
            <div className="space-y-4 relative z-10">
              {overallStats.top5Teams.slice(0, 3).map((t, idx) => (
                <div key={t.teamName} className="flex items-center gap-3 sm:gap-4 p-3 rounded-2xl border border-slate-200 bg-white hover:bg-slate-50 hover:-translate-y-0.5 transition-all duration-200 shadow-sm min-w-0">
                  <div className="shrink-0">
                    <RankBadge rank={idx + 1} size={30} />
                  </div>
                  <div className="grow min-w-0">
                    <h4 className="font-black text-sm text-[#0B1B3A] truncate" title={t.teamName}>{t.teamName}</h4>
                    <p className="text-[11px] text-slate-600 font-extrabold uppercase truncate tracking-wider mt-0.5">{t.department}</p>
                    {t.leadingProductTitle && (
                      <p className="text-xs text-slate-700 font-medium italic truncate mt-1 bg-slate-50 px-2 py-0.5 rounded border border-slate-200" title={t.leadingProductTitle}>
                        {t.leadingProductTitle}
                      </p>
                    )}
                  </div>
                  <div className="text-right shrink-0">
                    {t.highestTrl !== undefined && t.highestTrl !== null ? (
                      <>
                        ...
                        <p className="text-[10px] text-slate-600 font-bold uppercase tracking-wider mt-1">Status</p>
                      </>
                    ) : (
                      <>
                        <span className="text-base font-black text-[#0B1B3A]">{t.ideas}</span>
                        <p className="text-[10px] text-slate-600 font-bold uppercase tracking-wider">Ideas</p>
                      </>
                    )}
                  </div>
                </div>
              ...
              <table className="w-full border-collapse text-left text-sm text-slate-700 table-fixed">
                <thead className="bg-slate-100/90 text-[11px] font-black text-slate-700 uppercase border-b border-slate-200 tracking-wider">
                  <tr>
                    ...
                    <th className="px-4 py-3 w-1/2">{hasTrlData ? 'Team & Product Assembly' : 'Team'}</th>
                    <th className="px-4 py-3 w-1/3">Department</th>
                    <th className="px-4 py-3 text-right w-28">{hasTrlData ? 'Highest TRL' : 'Ideas'}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-150 bg-white">
                  {paginatedTeams.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.teamName} className="hover:bg-slate-50 transition-all duration-200 group">
                        <td className="px-4 py-3 text-center">
                          <RankBadge rank={globalRank} size={28} />
                        </td>
                        <td className="px-4 py-3 min-w-0">
                          <div className="font-black text-[#0B1B3A] truncate" title={row.teamName}>{row.teamName}</div>
                          {row.leadingProductTitle && (
                            <div className="text-xs text-slate-700 font-medium italic truncate mt-0.5" title={row.leadingProductTitle}>
                              {row.leadingProductTitle}
                            </div>
                          )}
                        </td>
                        <td className="px-4 py-3 text-xs font-bold text-slate-700 truncate uppercase tracking-wider" title={row.department}>
                          {row.department}
                        </td>
                        <td className="px-4 py-3 text-right font-black">
                          {row.highestTrl !== undefined && row.highestTrl !== null ? (
                            ...
                            <span className="font-black text-[#0B1B3A] text-base">{row.ideas}</span>
                          )}
                        </td>
                      </tr>
                    )
                    ...
                    <div className="flex items-start justify-between gap-3 min-w-0">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h4 className="font-black text-sm text-[#0B1B3A] break-words">{row.teamName}</h4>
                          <p className="text-xs text-slate-600 font-bold uppercase tracking-wider mt-0.5 break-words">{row.department}</p>
                        </div>
                      </div>
  ...
  const renderDepartmentView = () => {
    ...
    const departmentsOptions = Array.from(new Set(overallStats.allTeams.map(t => t.department).filter(Boolean))).sort((a, b) => String(a ?? '').localeCompare(String(b ?? '')));
      ...
      <div className="space-y-8 mt-8 relative">
        ...
        <div className="relative bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm overflow-hidden z-10 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-4 mb-6 gap-4 relative z-10">
            <div>
              <h2 className="text-base sm:text-lg font-black text-[#0B1B3A] font-heading tracking-widest uppercase">DEPARTMENT LEADERBOARD</h2>
              <p className="text-xs text-slate-600 mt-0.5 font-bold uppercase tracking-wider">Rankings and analytics based on highest TRL achieved by teams</p>
            </div>

            <div className="flex items-center gap-3 shrink-0 flex-wrap">
              <MechanicalGearsMini scrollProgress={scrollProgress} />
              <label htmlFor="dept-tab-filter" className="text-[11px] font-black text-slate-700 uppercase tracking-wider">Department:</label>
              <select
                id="dept-tab-filter"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 relative z-10">
            <div className="space-y-4 min-w-0">
                ...
                <table className="w-full border-collapse text-left text-sm text-slate-700 table-fixed">
                  ...
                  <tbody className="divide-y divide-slate-150 bg-white">
                    {filteredRankedDepts.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="px-4 py-8 text-center text-xs italic text-slate-600 bg-slate-50">
                          No departments match the criteria.
                        </td>
                      </tr>
                    ) : (
                      filteredRankedDepts.map((row) => (
                        <tr key={row.department} className="hover:bg-slate-50 transition-all duration-200 group">
                          <td className="px-3 py-3 text-center">
                            <RankBadge rank={row.rank} size={28} />
                          </td>
                          <td className="px-3 py-3 font-black text-[#0B1B3A] truncate" title={row.department}>
                            {row.department}
                          </td>
                          <td className="px-3 py-3 text-right font-black text-[#0B1B3A]">{row.teamsCount}</td>
                          <td className="px-3 py-3 text-right">
                            <span className="inline-flex items-center justify-center bg-primary/15 text-primary font-mono font-black text-xs px-2.5 py-0.5 rounded border border-primary/30 shadow-2xs">
                              TRL {row.highestTrl}
                            </span>
                          </td>
                          <td className="px-3 py-3 text-right font-black text-[#0B1B3A]">{row.ideasCount}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
              ...
              <div className="block md:hidden space-y-3 w-full min-w-0">
                {filteredRankedDepts.map((row) => (
                  <div key={row.department} className="p-4 rounded-2xl border border-slate-200 bg-slate-50/50 space-y-2.5 shadow-2xs min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-3 min-w-0">
                        <RankBadge rank={row.rank} size={30} />
                        <h4 className="font-black text-sm text-[#0B1B3A] break-words">{row.department}</h4>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200 text-xs">
                      <div className="bg-white p-2 rounded-xl border border-slate-200 text-center">
                        <span className="text-[10px] font-bold text-slate-600 uppercase block">Teams</span>
                        <span className="font-black text-[#0B1B3A] text-sm">{row.teamsCount}</span>
                      </div>
                      <div className="bg-white p-2 rounded-xl border border-slate-200 text-center">
                        <span className="text-[10px] font-bold text-slate-600 uppercase block">Ideas</span>
                        <span className="font-black text-[#0B1B3A] text-sm">{row.ideasCount}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-4 flex flex-col justify-between min-w-0">
              <div>
                ...
                <p className="text-xs text-slate-600 font-bold uppercase tracking-wider mb-6">Department-wise team distribution across TRL levels</p>
                ...
                <div className="w-full min-w-0">
                  <div className="flex justify-between items-end h-56 sm:h-64 w-full px-1 pb-2 relative gap-1 sm:gap-2">
                    ...
                    {filteredRankedDepts.map(d => {
                      ...
                      return (
                        <div key={d.department} className="flex flex-col items-center gap-1.5 flex-1 min-w-0 relative z-10">
                          <span className="text-[9px] font-black text-[#0B1B3A] font-mono">{totalTeams}</span>
                          ...
                          <div
                            ...
                            style={{ height: `${Math.max((totalTeams / maxTeamsInTrlDist) * 150, 20)}px` }}
                          >
                          ...
                          <span className="text-[7px] sm:text-[9px] font-black text-[#0B1B3A] text-center truncate w-full uppercase tracking-wider px-0.5" title={d.department}>
                            {d.department}
                          </span>
                        </div>
                      );
  ...
  const renderDomainView = () => {
    ...
    const domainsOptions = Array.from(new Set(overallStats.allTeams.map(t => t.innovationDomain).filter(Boolean))).sort((a, b) => String(a ?? '').localeCompare(String(b ?? '')));
    ...
    return (
      <div className="space-y-8 mt-8 relative">
        ...
        <div className="relative bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm overflow-hidden z-10 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-4 mb-6 gap-4 relative z-10">
            <div>
              <h2 className="text-base sm:text-lg font-black text-[#0B1B3A] font-heading tracking-widest uppercase">DOMAIN LEADERBOARD</h2>
              <p className="text-xs text-slate-600 mt-0.5 font-bold uppercase tracking-wider">Rankings and analytics based on highest TRL achieved by teams</p>
            </div>

            <div className="flex items-center gap-3 shrink-0 flex-wrap">
              <MechanicalGearsMini scrollProgress={scrollProgress} />
              <label htmlFor="dom-tab-filter" className="text-[11px] font-black text-slate-700 uppercase tracking-wider">Domain:</label>
              <select
                id="dom-tab-filter"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 relative z-10">
            <div className="space-y-4 min-w-0">
                  ...
                  <tbody className="divide-y divide-slate-150 bg-white">
                    {filteredRankedDoms.length === 0 ? (
                      ...
                      filteredRankedDoms.map((row) => (
                        <tr key={row.domain} className="hover:bg-slate-50 transition-all duration-200 group">
                          <td className="px-3 py-3 text-center">
                            <RankBadge rank={row.rank} size={28} />
                          </td>
                          <td className="px-3 py-3 font-black text-[#0B1B3A] truncate" title={row.domain}>
                            {row.domain}
                          </td>
                          <td className="px-3 py-3 text-right font-black text-[#0B1B3A]">{row.teamsCount}</td>
                          <td className="px-3 py-3 text-right">
              ...
              <div className="block md:hidden space-y-3 w-full min-w-0">
                {filteredRankedDoms.map((row) => (
                  <div key={row.domain} className="p-4 rounded-2xl border border-slate-200 bg-slate-50/50 space-y-2.5 shadow-2xs min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-3 min-w-0">
                        <RankBadge rank={row.rank} size={30} />
                        <h4 className="font-black text-sm text-[#0B1B3A] break-words">{row.domain}</h4>
                      </div>
                      <span className="shrink-0 inline-flex items-center justify-center bg-primary/15 text-primary font-mono font-black text-xs px-2.5 py-1 rounded border border-primary/30">
                        TRL {row.highestTrl}
                      </span>
            ...
            <div className="space-y-4 flex flex-col justify-between min-w-0">
              <div>
                ...
                <p className="text-xs text-slate-600 font-bold uppercase tracking-wider mb-6">Domain-wise team distribution across TRL levels</p>
                ...
                <div className="w-full min-w-0">
                  <div className="flex justify-between items-end h-56 sm:h-64 w-full px-1 pb-2 relative gap-1 sm:gap-2">
                    ...
                    {filteredRankedDoms.map(d => {
                      const totalTeams = d.teamsCount;
                      return (
                        <div key={d.domain} className="flex flex-col items-center gap-1.5 flex-1 min-w-0 relative z-10">
                          <span className="text-[9px] font-black text-[#0B1B3A] font-mono">{totalTeams}</span>
                          ...
                          <span className="text-[7px] sm:text-[9px] font-black text-[#0B1B3A] text-center truncate w-full uppercase tracking-wider px-0.5" title={d.domain}>
                            {d.domain}
                          </span>
                        </div>
                      );
    ...
    <div className="bg-slate-50 min-h-screen pb-20 pt-24 relative overflow-x-hidden">
      <div className="bg-slate-900 text-white py-12 md:py-16 relative overflow-hidden rounded-b-3xl">
        ...
        <div className="mx-auto w-full max-w-[1400px] px-4 md:px-6 lg:px-8 relative z-10 text-center">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-amber-300 text-xs font-black uppercase tracking-widest border border-white/20 mb-3 backdrop-blur-xs">
            {leaderboardType === 'VOTING_BASED' ? 'Voting Based Evaluation' : 'TRL Based Evaluation'}
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-8 sm:p-14 text-center shadow-sm max-w-2xl mx-auto space-y-4">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600 font-bold max-w-md mx-auto leading-relaxed">
                The community voting evaluation round has not opened. Team rankings will be calculated and displayed once community voting begins.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 min-[360px]:grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-5 mb-8">
              ...
              <div className="relative bg-white border border-slate-200 p-4 sm:p-5 rounded-2xl shadow-sm flex flex-col justify-between hover:shadow-md hover:border-slate-300 transition overflow-hidden group min-w-0">
                ...
                <div className="flex items-center justify-between mb-3 relative z-10">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 truncate">Teams Registered</span>
                  <div className="p-1.5 rounded-lg bg-blue-500/10 text-primary border border-blue-200/50 shadow-sm shrink-0"><Users size={16} /></div>
                </div>
                <div className="relative z-10">
                  <span className="text-2xl font-black text-[#0B1B3A] tracking-tight">{kpis.totalTeams}</span>
                  <p className="text-[10px] text-slate-600 mt-1 font-bold uppercase">Unique teams</p>
                </div>
              </div>
              ...
              <div className="relative bg-white border border-slate-200 p-4 sm:p-5 rounded-2xl shadow-sm flex flex-col justify-between hover:shadow-md hover:border-slate-300 transition overflow-hidden group min-w-0">
                ...
                <div className="flex items-center justify-between mb-3 relative z-10">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 truncate">Students Participating</span>
                  <div className="p-1.5 rounded-lg bg-amber-500/10 text-accent border border-amber-200/50 shadow-sm shrink-0"><Award size={16} /></div>
                </div>
                <div className="relative z-10">
                  <span className="text-2xl font-black text-[#0B1B3A] tracking-tight">{kpis.totalStudents}</span>
                  <p className="text-[10px] text-slate-600 mt-1 font-bold uppercase">Unique students</p>
                </div>
              </div>
              ...
              <div className="relative bg-white border border-slate-200 p-4 sm:p-5 rounded-2xl shadow-sm flex flex-col justify-between hover:shadow-md hover:border-slate-300 transition overflow-hidden group min-w-0">
                ...
                <div className="flex items-center justify-between mb-3 relative z-10">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 truncate">Ideas Submitted</span>
                  <div className="p-1.5 rounded-lg bg-purple-500/10 text-purple-600 border border-purple-200/50 shadow-sm shrink-0"><Layers size={16} /></div>
                </div>
                <div className="relative z-10">
                  <span className="text-2xl font-black text-[#0B1B3A] tracking-tight">{kpis.totalIdeas}</span>
                  <p className="text-[10px] text-slate-600 mt-1 font-bold uppercase">Ideas submitted</p>
                </div>
              </div>
              ...
              <div className="relative bg-white border border-slate-200 p-4 sm:p-5 rounded-2xl shadow-sm flex flex-col justify-between hover:shadow-md hover:border-slate-300 transition overflow-hidden group min-w-0">
                ...
                <div className="flex items-center justify-between mb-3 relative z-10">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 truncate">Departments</span>
                  <div className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 border border-emerald-200/50 shadow-sm shrink-0"><Building size={16} /></div>
                </div>
                <div className="relative z-10">
                  <span className="text-2xl font-black text-[#0B1B3A] tracking-tight">{kpis.totalDepartments}</span>
                  <p className="text-[10px] text-slate-600 mt-1 font-bold uppercase">Active departments</p>
                </div>
              </div>
              ...
              <div className="relative bg-white border border-slate-200 p-4 sm:p-5 rounded-2xl shadow-sm flex flex-col justify-between hover:shadow-md hover:border-slate-300 transition overflow-hidden group min-w-0">
                ...
                <div className="flex items-center justify-between mb-3 relative z-10">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 truncate">Innovation Domains</span>
                  <div className="p-1.5 rounded-lg bg-orange-500/10 text-orange-600 border border-orange-200/50 shadow-sm shrink-0"><Cpu size={16} /></div>
                </div>
                <div className="relative z-10">
                  <span className="text-2xl font-black text-[#0B1B3A] tracking-tight">{kpis.totalDomains}</span>
                  <p className="text-[10px] text-slate-600 mt-1 font-bold uppercase">Innovation domains</p>
                </div>
              </div>
            </div>
              ...
              {loading ? (
                <div className="flex h-96 items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white shadow-sm">
                  <div className="text-center">
                    ...
                    <p className="mt-3 text-sm text-slate-600 font-bold uppercase tracking-wider">Aggregating live metrics...</p>
                  </div>
                </div>
              ) : error ? (
                ...
                <div className="flex h-96 items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white shadow-sm">
                  <div className="text-center text-slate-600">
                    ...
                    <p className="mt-1 text-xs text-slate-500">Leaderboard statistics will show here once teams submit ideas.</p>
                  </div>
                </div>
              ) : (
